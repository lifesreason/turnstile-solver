...
class DeploymentFootprintTest(unittest.TestCase):
    ...
    def test_runtime_defaults_to_reused_idle_reclaimed_browser_pool(self):
        ...
        self.assertIn("TURNSTILE_BROWSER_RECYCLE_TASKS=25", dockerfile)
        self.assertIn("TURNSTILE_BROWSER_RECYCLE_RSS_MB=800", dockerfile)
        self.assertIn("TURNSTILE_POOL_ACQUIRE_TIMEOUT_SEC=30", dockerfile)
        ...
        self.assertIn('TURNSTILE_POOL_ACQUIRE_TIMEOUT_SEC: "${TURNSTILE_POOL_ACQUIRE_TIMEOUT_SEC:-30}"', compose)
        ...
        self.assertIn("TURNSTILE_BROWSER_RECYCLE_TASKS=25", env_example)
        self.assertIn("TURNSTILE_BROWSER_RECYCLE_RSS_MB=800", env_example)
        self.assertIn("TURNSTILE_POOL_ACQUIRE_TIMEOUT_SEC=30", env_example)
        self.assertIn("`TURNSTILE_KEEP_BROWSER_ALIVE` | `1`", readme)
        self.assertIn("`TURNSTILE_BROWSER_RECYCLE_TASKS` | `25`", readme)
        self.assertIn("`TURNSTILE_BROWSER_RECYCLE_RSS_MB` | `800`", readme)
        self.assertIn("`TURNSTILE_POOL_ACQUIRE_TIMEOUT_SEC` | `30`", readme)
