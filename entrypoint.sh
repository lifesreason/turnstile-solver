...
POOL_ACQUIRE_TIMEOUT="${TURNSTILE_POOL_ACQUIRE_TIMEOUT_SEC:-120}"
SOLVE_TIMEOUT="${TURNSTILE_SOLVE_TIMEOUT_SEC:-60}"
MAX_PENDING="${TURNSTILE_MAX_PENDING_TASKS:-2}"
HEALTH_STUCK="${TURNSTILE_HEALTH_STUCK_SEC:-180}"
DEBUG_FLAG=()
...
echo "[turnstile-solver] browser=${BROWSER_TYPE} profile=${TURNSTILE_CAMOUFOX_PROFILE:-compact} concurrency_slots=${THREAD} browser_instances=${INSTANCES} worker_mode=${TURNSTILE_WORKER_MODE:-inline} keep_alive=${KEEP_ALIVE} idle=${TURNSTILE_IDLE_SEC:-300}s recycle_tasks=${TURNSTILE_BROWSER_RECYCLE_TASKS:-8} recycle_rss_mb=${TURNSTILE_BROWSER_RECYCLE_RSS_MB:-700} pool_acquire_timeout=${POOL_ACQUIRE_TIMEOUT}s solve_timeout=${SOLVE_TIMEOUT}s max_pending=${MAX_PENDING} health_stuck=${HEALTH_STUCK}s ${HOST}:${PORT} lazy=${TURNSTILE_LAZY:-1}"
